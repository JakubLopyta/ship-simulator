...
public class TestModel : MonoBehaviour, IModel
{
	...
	[SerializeField] private double HeadingRad = 0f;
	[SerializeField] private double YawRateRad = 0f;
	[SerializeField] private double currentRudderDeg;

	[Header("Surge tuning")]
	[Tooltip("N at enginePower = 1")]
	[SerializeField] private double thrustForce = 5e5f;
	[Tooltip("Extra drag multiplier for rudder angle^2 (1.0-3.0)")]
	[SerializeField] private double rudderDragK = 1.8f;    
	
	[Header("Yaw tuning")]
	[Tooltip("How strongly rudder turns the ship")]
	[SerializeField] private double yawGain = 0.5f;
	[Tooltip("How fast yawRate reaches target rate in rad/s^2")]
	[SerializeField] private double YawAccel = 5e-3f;

	[Space(10)]
	[Tooltip("Maximum rudder deviation in deg")]
	[SerializeField] private double rudderMax = 35f;
	...
	public void Calculate()
	{
		...
		double dragCoefficient0 = (vmax != 0f) ? (thrustForce / (vmax * vmax)) : 0f;
